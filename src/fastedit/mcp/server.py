...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "includes a caller-impact note listing counts — informational only, does not "
        "block the edit.\n"
        ...
        "- fast_delete: Remove a function/class/method by name. Runs a "
        "cross-file caller check via tldr references first — refuses to "
        "delete if the symbol has references in other files. Pass "
        "force=True to override. Instant, no model.\n"
        ...
        "- fast_rename: Rename all occurrences of a symbol in a single "
        "file. AST-verified via tldr references (skips string/comment "
        "substring matches). Pass dry_run=True for a preview. Instant, "
        "no model.\n"
        "- fast_rename_all: Cross-file symbol rename. AST-verified via "
        "tldr references — skips strings/comments structurally. Optional "
        "kind_filter ('class'|'function'|'method'|'variable') narrows "
        "to a specific definition kind. Walks every supported code file "
        "under a directory, pruning .git, node_modules, __pycache__, "
        "target, dist, vendor, and other common vendor/build dirs. Pass "
        "dry_run=True to preview. Instant, no model.\n"
        "- fast_move_to_file: Relocate a symbol from one file to another. "
        "Automatically rewrites import statements in every consumer file "
        "via tldr references + per-language rewriters. Pass dry_run=True "
        "for a preview. All 13 fastedit languages supported (python, "
        "javascript, typescript, rust, go, java, c, c++, ruby, swift, "
        "kotlin, c#, php; plus scala, elixir, lua when present). "
