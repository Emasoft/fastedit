"""vLLM-backed inference adapter for FastEdit.

This is a thin OpenAI-compatible client that matches the subset of the
``MLXEngine`` interface used by the chunked merge + MCP runtime.
"""
...
class VLLMEngine:
    """OpenAI-compatible adapter for a FastEdit model served by vLLM."""
    ...
    def merge_auto(
        ...
        """vLLM already handles scheduling/batching, so merge_auto == merge."""
